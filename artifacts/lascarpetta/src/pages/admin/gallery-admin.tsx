  ...
  return (
    <AdminLayout>
      <div className="p-6">
        ...
        {showForm && (
          <form onSubmit={handleSubmit(onSubmit)} className="bg-card border border-border p-6 mb-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
            ...
            <div>
              <label className="block text-sm font-medium mb-1">Caption (RU)</label>
              <Input {...register("captionRu")} className="rounded-none" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Caption (EN)</label>
              <Input {...register("captionEn")} className="rounded-none" />
            </div>
            <div className="sm:col-span-2 flex gap-2">
              <Button type="submit" className="rounded-none" disabled={createPhoto.isPending}>Add</Button>
        ...
        <div className="flex gap-3 mb-6 flex-wrap">
          <button onClick={() => setTypeFilter("all")} className={`px-4 py-1.5 text-sm rounded-none border ${typeFilter === "all" ? "bg-primary text-white border-primary" : "border-border"}`}>All</button>
          {TYPES.map(t => (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
          {photos?.map((p) => (
            <div key={p.id} className="relative group aspect-square overflow-hidden border border-border">
              ...
              <span className="absolute bottom-0 left-0 right-0 bg-black/60 text-white text-xs p-1 truncate">{p.type}</span>
            </div>
          ))}
        </div>

        {photos?.length === 0 && <p className="text-center py-12 text-muted-foreground">No photos. Add some!</p>}
      </div>
    </AdminLayout>
  );
}
